<section class="p-top-blog">
    <div class="l-inner">
        <h2 class="c-section-title" data-align="left">
            <span class="c-section-title--ja">採用ブログ</span>
            <span lang="en" aria-hidden="true" class="c-section-title--en">Blog</span>
        </h2>
        <p class="p-top-blog__text">
            採用情報やイベント情報、社員の紹介など、<br>
            日々の現場の様子をご紹介します。
        </p>
        <div class="p-top-blog__button-wrap">
            <a href="<?php echo esc_url(home_url("/")); ?>" class="p-top-blog__link">
            <svg class="p-top-blog__link-icon" aria-hidden="true" focusable="false" width="58" height="58" viewBox="0 0 58 58" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="29" cy="29" r="28.5" fill="white" stroke="#222222"/>
                <g transform="translate(20.5, 21.5)">
                    <path d="M16.7071 8.07112C17.0976 7.6806 17.0976 7.04743 16.7071 6.65691L10.3431 0.292946C9.95262 -0.0975785 9.31946 -0.0975785 8.92893 0.292946C8.53841 0.68347 8.53841 1.31664 8.92893 1.70716L14.5858 7.36401L8.92893 13.0209C8.53841 13.4114 8.53841 14.0446 8.92893 14.4351C9.31946 14.8256 9.95262 14.8256 10.3431 14.4351L16.7071 8.07112ZM0 8.36401H16V6.36401H0V8.36401Z" fill="#222222"/>
                </g>
            </svg>
            <span class="p-top-blog__link-text">View more</span>
            <span class="u-sr-only">採用ブログページへ</span>
        </a>
    </div>

       <ul class="p-top-blog__list">
  <li class="p-top-blog__item">
    <a href="<?php echo esc_url(home_url("/")); ?>" class="p-top-blog__item-link">
      <div class="p-top-blog__img">
        <img
          src="<?php echo esc_url(get_theme_file_uri("/assets/images/image_blog_01.webp")); ?>"
          alt=""
          width="156"
          height="180"
          loading="lazy"
        >
      </div>
      <div class="p-top-blog__content">
        <div class="c-category">
          <?php include get_theme_file_path("/assets/images/image_category.svg"); ?>
          <span class="c-category__text">社内研修</span>
        </div>
        <h3 class="p-top-blog__title">新入社員向けに、入社前研修を行いました。</h3>
        <time class="p-top-blog__date" datetime="2025-03-25">2025.03.25</time>
      </div>
    </a>
  </li>
  <li class="p-top-blog__item">
    <a href="<?php echo esc_url(home_url("/")); ?>" class="p-top-blog__item-link">
      <div class="p-top-blog__img">
        <img
          src="<?php echo esc_url(get_theme_file_uri("/assets/images/image_blog_02.webp")); ?>"
          alt=""
          width="156"
          height="180"
          loading="lazy"
        >
      </div>
      <div class="p-top-blog__content">
        <div class="c-category">
          <?php include get_theme_file_path("/assets/images/image_category.svg"); ?>
          <span class="c-category__text">社内研修</span>
        </div>
        <h3 class="p-top-blog__title">内定者向け研修を行いました。</h3>
        <time class="p-top-blog__date" datetime="2024-08-25">2024.08.25</time>
      </div>
    </a>
  </li>
  <li class="p-top-blog__item">
    <a href="<?php echo esc_url(home_url("/")); ?>" class="p-top-blog__item-link">
      <div class="p-top-blog__img">
        <img
          src="<?php echo esc_url(get_theme_file_uri("/assets/images/image_blog_03.webp")); ?>"
          alt=""
          width="156"
          height="180"
          loading="lazy"
        >
      </div>
      <div class="p-top-blog__content">
        <div class="c-category">
          <?php include get_theme_file_path("/assets/images/image_category.svg"); ?>
          <span class="c-category__text">社内イベント</span>
        </div>
        <h3 class="p-top-blog__title">テキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキスト</h3>
        <time class="p-top-blog__date" datetime="2024-03-25">2024.03.25</time>
      </div>
    </a>
  </li>
  <li class="p-top-blog__item">
    <a href="<?php echo esc_url(home_url("/")); ?>" class="p-top-blog__item-link">
      <div class="p-top-blog__img">
        <img
          src="<?php echo esc_url(get_theme_file_uri("/assets/images/image_blog_04.webp")); ?>"
          alt=""
          width="156"
          height="180"
          loading="lazy"
        >
      </div>
      <div class="p-top-blog__content">
        <div class="c-category">
          <?php include get_theme_file_path("/assets/images/image_category.svg"); ?>
          <span class="c-category__text">お知らせ</span>
        </div>
        <h3 class="p-top-blog__title">テキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキスト</h3>
        <time class="p-top-blog__date" datetime="2024-03-01">2024.03.01</time>
      </div>
    </a>
  </li>
</ul>
    </div>
</section>