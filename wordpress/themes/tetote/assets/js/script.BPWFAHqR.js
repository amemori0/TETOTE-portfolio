document.addEventListener("DOMContentLoaded", () => {
  const header2 = document.querySelector(".p-header");
  ...
  const closeMenu = () => {
    header2.classList.remove("js-open");
  ...
  const clickOuter = (event) => {
    if (drawer.classList.contains("js-open") && !drawer.contains(event.target) && !drawerIcon.contains(event.target) && isMenuOpen) {
      closeMenu();
    } else if (drawer.classList.contains("js-open") && !drawer.contains(event.target)) {
      isMenuOpen = true;
    }
  };
...
var Observer$1 = /* @__PURE__ */ function() {
  ...
  _proto.init = function init4(vars) {
    ...
    var id2, onStopDelayedCall, dragged, moved, wheeled, locked, axis, self = this, prevDeltaX = 0, prevDeltaY = 0, passive = vars.passive || !preventDefault && vars.passive !== false, scrollFuncX = _getScrollFunc(target, _horizontal), scrollFuncY = _getScrollFunc(target, _vertical), scrollX = scrollFuncX(), scrollY = scrollFuncY(), limitToTouch = ~type.indexOf("touch") && !~type.indexOf("pointer") && _eventTypes[0] === "pointerdown", isViewport = _isViewport$1(target), ownerDoc = target.ownerDocument || _doc$1, deltaX = [0, 0, 0], deltaY = [0, 0, 0], onClickTime = 0, clickCapture = function clickCapture2() {
      ...
      self._vx.update(x);
      self._vy.update(y);
      debounce ? id2 || (id2 = requestAnimationFrame(update2)) : update2();
...
Observer$1.getById = function(id2) {
    ...
    if (gsap) {
      ...
      if (_body) {
        ...
        var onLoad2 = function onLoad3() {
          ...
          _doc.removeEventListener("DOMContentLoaded", onLoad3);
        };
        _doc.addEventListener("DOMContentLoaded", onLoad2);
      }
    }
  };
...
new Swiper(".p-top-staff__swiper", {
...
const initializeSmoothScroll = () => {
  document.addEventListener("click", handleClick$1, { capture: true });
};
const getHeaderBlockSize = () => {
  const header2 = document.querySelector("[data-fixed-header]");
  if (!header2)
    return "0";
  const { position, blockSize } = window.getComputedStyle(header2);
...
const observeHeaderBlockSize = new ResizeObserver((entries) => {
  const header2 = entries[0];
  if (header2.contentBoxSize) {
    const blockSize = header2.borderBoxSize[0].blockSize;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (header2)
    observeHeaderBlockSize.observe(header2);
...
const HEADING_SELECTOR = ".p-staff-detail-content__main h2, .p-staff-detail-content__main h3";
...
try {
  ...
  const findParentElement = (layer2, rank, diff) => {
    do {
      rank += diff;
      if (layer2[rank])
        return layer2[rank];
  ...
  headingElements.forEach((el) => {
    ...
    if (!layer[rank]) {
      ...
      layer[rank].className = `list_${rank}`;
...
const observerCallback = (entries, observer2) => {
  entries.forEach((entry) => {
    ...
    const tocLink = document.querySelector(`#toc a[href="#${id2}"]`);
...
const toggleAccordion = (details, panel, options, detailsName, show) => {
  ...
  const keyframes = show ? [{ maxBlockSize: "0" }, { maxBlockSize: blockSize }] : [{ maxBlockSize: blockSize }, { maxBlockSize: "0" }];
